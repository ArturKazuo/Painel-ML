...
async function sendMessage(activities, port = 60051) {
...
function sendText(phone, text, qrcode, port = 60051) {
...
function getPhoneNumber(phone, port = 60051) {
    if (!phone) return;
...
async function getWhatsappRequest(req, text, port = 60051) {
    ...
    let celular = body.cliente.celular
    let celularId = await getPhoneNumber(body?.cliente?.celular, port) //body.cliente.celular + "@c.us"
    let callback_url = body.callback_url
    let senderCelularId = "553172283951@c.us";
    const senderCelularName = empresa || "Flash";
...
function checkSim(text){
    ...
    if(text.match(new RegExp(/sim/i)) || text.match(new RegExp(/correto/i)) || text.match(new RegExp(/confirmo/i)) || text.match(new RegExp(/confirma/i)) || text.match(new RegExp(/ok/i))){
        return true;
    }
...
function checkPare(text){
    ...
    if(text.match(new RegExp(/pare/i)) || text.match(new RegExp(/parar/i)) || text.match(new RegExp(/cancelar/i)) || text.match(new RegExp(/cancela/i)) ){
        return true;
    }
